...
def downscale(targetVar, methodName, family, mode, fields, scene, model):
    ...
    if scene == 'TESTING':
        ...
        X_test = aux['data'][idates]
        times = aux['times'][idates]
        X_test = transform.transform(targetVar, X_test, times, model, 'spred')
        X_test = X_test.astype('float32')
    ...
    if methodName == 'UNET':
        (X_test, y_train,
         ...
         hX_adj, wX_adj, nPointsY_adj) = aux_lib.adjust_dimensions_for_UNET(X_test,
                                                       hX_orig=X_test.shape[2], wX_orig=X_test.shape[3],
                                                       nPointsY_orig=hres_npoints[targetVar])
    ...
    if methodName == 'UNET':
        y_shape = (X_test.shape[0], nPointsY_adj)
    else:
    ...
    if targetVar == 'pr':
        if methodName == 'DeepESD':
            ...
            model_deep = deep_models.UnetPr(x_shape=X_test.shape, y_shape=y_shape, stochastic=False,
    ...
    # Create template
    template = xr.Dataset(
        {targetVar: (["point"], np.ones(y_shape[1], dtype=np.int8))},
        coords={"point": np.arange(y_shape[1])},
    )
    ...
    est[:] = np.nan
    est[valid] = y_pred[:, :hres_npoints[targetVar]]


    # Adapt shapes for UNET
    if methodName == 'UNET':
        X_test, est = aux_lib.adjust_dimensions_for_UNET(X_test, Y=est,
                                   hX_orig=hX_orig, wX_orig=wX_orig, nPointsY_orig=nPointsY_orig,
                                   hX_adj=hX_adj, wX_adj=wX_adj, nPointsY_adj=nPointsY_adj,
                                   revert_change=True)
