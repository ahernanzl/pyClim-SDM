...
def downscale_chunk(var, methodName, family, mode, fields, scene, model, iproc=0, nproc=1):
    ...
    for ipoint in range(len(points_chunk[ichunk])):
        ...
        # Apply downscaling
        if methodName == 'QM':
            est[:, ipoint_local_index] = MOS_lib.quantile_mapping(Y_train, X_train, X_test, var)[:, 0]
        if methodName == 'DQM':
            est[:, ipoint_local_index] = MOS_lib.detrended_quantile_mapping(Y_train, X_train, X_test, var)[:, 0]
        if methodName == 'QDM':
            est[:, ipoint_local_index] = MOS_lib.quantile_delta_mapping(Y_train, X_train, X_test, var)[:, 0]
        elif methodName == 'PSDM':
            est[:, ipoint_local_index] = MOS_lib.scaled_distribution_mapping(Y_train, X_train, X_test, var)[:, 0]
